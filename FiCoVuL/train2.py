...
class JointLoss(torch.nn.Module):
    def __init__(self, alpha=0.85):
        ...
        self.n_loss = FocalLoss()

    def forward(self, g_predict, g_target, n_predict, n_target):
        # print(g_predict.shape, g_target.shape, n_predict.shape, n_target.shape)
        g_loss = self.g_loss(g_predict, g_target)
        filtered_indices = torch.nonzero(n_target != -1.).squeeze()
        n_predict = n_predict[filtered_indices]
        n_target = n_target[filtered_indices]
        n_loss = self.n_loss(n_predict, n_target)
        loss = self.alpha * g_loss + (1 - self.alpha) * n_loss * 50
        return loss
...
def train_gat(config):
    ...
    loss_fn = JointLoss(alpha=config['run']['alpha']).to(device)
    # loss_fn = BCEFocalLoss().to(device)
...
def get_main_loop(config, gat, loss_func, optimizer, patience_period, time_start):
    ...
    def main_loop(phase, data_loader, epoch=0):
        ...
        for mini_batch_idx, (node_features, gt_graph_labels, edge_index, node_to_graph_map, rpst_labels) in enumerate(bar):
            ...
            graph_unnormalized_scores = graph_unnormalized_scores.squeeze(-1)
            node_unnormalized_scores = node_unnormalized_scores.squeeze(-1)
            # print("2", graph_unnormalized_scores.shape, gt_graph_labels.shape, node_unnormalized_scores.shape, rpst_labels.shape)
            loss = loss_func(graph_unnormalized_scores, gt_graph_labels, node_unnormalized_scores, rpst_labels)
            ...
            total_graph_preds.extend((graph_unnormalized_scores > label_theshold).int().cpu().tolist())
            total_graph_gts.extend(gt_graph_labels.int().cpu().tolist())
            filter_node_unnormalized_scores = node_unnormalized_scores[rpst_labels != -1.]
            filter_rpst_labels = rpst_labels[rpst_labels != -1.]
            total_node_preds.extend((filter_node_unnormalized_scores > label_theshold).int().cpu().tolist())
            total_node_gts.extend(filter_rpst_labels.int().cpu().tolist())
            if is_show_this_time:
                # 一个 batch_size 的 loss 和
                batch_mean_loss = sum(total_loss_list[mini_batch_idx+1-mini_batch_size:]) / mini_batch_size
                batch_micro_f11 = f1_score(
                    np.array(total_graph_gts[(mini_batch_idx+1)*config['run']['loader_batch_size']-config['run']['batch_size']:]),
                    np.array(total_graph_preds[(mini_batch_idx+1)*config['run']['loader_batch_size']-config['run']['batch_size']:]),
                    average='binary')
                batch_node_gts = total_node_gts[(mini_batch_idx + 1) * config['run']['loader_batch_size'] - config['run']['batch_size']:]
                # batch_micro_f12 = f1_score(
                #     np.array(batch_node_gts),
                #     np.array(total_node_preds[(mini_batch_idx + 1) * config['run']['loader_batch_size'] - config['run']['batch_size']:]),
                #     average='weighted', sample_weight=compute_sample_weight(class_weight='balanced', y=batch_node_gts))
                batch_micro_f12 = f1_score(
                    np.array(batch_node_gts),
                    np.array(total_node_preds[(mini_batch_idx + 1) * config['run']['loader_batch_size'] - config['run']['batch_size']:]),
                    average='binary')
                bar.set_postfix({"last_update": last_update, "loss": batch_mean_loss, f"{phase.name}_g_f1": batch_micro_f11, f"{phase.name}_n_f1": batch_micro_f12})
                ...
                if config['run']['enable_tensorboard']:
                    ...
                    if phase == LoopPhase.TRAIN:
                        ...
                        writer.add_scalar('training_micro_f11', batch_micro_f11, global_step)
                        writer.add_scalar('training_micro_f12', batch_micro_f12, global_step)
                    elif phase == LoopPhase.VAL:
                        writer.add_scalar('val_loss', batch_mean_loss, global_step)
                        writer.add_scalar('val_micro_f11', batch_micro_f11, global_step)
                        writer.add_scalar('val_micro_f12', batch_micro_f12, global_step)
        ...
        precision = alpha * precision1 + (1-alpha) * precision2
        recall = alpha * recall1 + (1-alpha) * recall2
        accuracy = alpha * accuracy1 + (1-alpha) * accuracy2
        mean_micro_f1 = alpha * mean_micro_f11 + (1-alpha) * mean_micro_f12
