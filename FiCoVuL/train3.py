...
from models.GATClassification import GATBinaryClassification, GATDualClassification
...
class FocalLoss(nn.Module):
    ...
    def forward(self, logits, label):
        ...
        coeff = torch.clamp(coeff, min=float('-inf'), max=float('inf'))
...
def train_gat(config):
    ...
    gat = GATDualClassification(
        ...
        enable_global_exchange=config['model']['enable_global_exchange'],
    ...
    loss_fn_graph = nn.BCEWithLogitsLoss().to(device)
    optimizer_graph = Adam(gat.parameters(), lr=config['run']['lr'], weight_decay=config['run']['weight_decay'])
    scheduler_graph = lr_scheduler.CosineAnnealingLR(optimizer_graph, config['run']['num_of_epochs'], eta_min=0)
    loss_fn_node = FocalLoss(alpha=config['run']['alpha']).to(device)
    node_classifier_params = list(map(id, gat.node_classifier.parameters()))
    base_params = filter(lambda p: id(p) not in node_classifier_params, gat.parameters())
    amplifier = 20
    optimizer_node = Adam([{'params': base_params}, {'params': gat.node_classifier.parameters(), 'lr': config['run']['lr']*amplifier}],
                          lr=config['run']['lr']*amplifier/10, weight_decay=config['run']['weight_decay'])
    scheduler_node = lr_scheduler.CosineAnnealingLR(optimizer_graph, config['run']['num_of_epochs'], eta_min=0)

    # The decorator function makes things cleaner since there is a lot of redundancy between the train and val loops
    main_loop_graph = get_main_loop(
        config,
        gat,
        loss_fn_graph,
        optimizer_graph,
    ...
    for epoch in bar:
        print('=' * 15 + f" * EPOCH \t{epoch:4}\tgraph" + '=' * 15)
        # Training loop
        main_loop_graph(phase=LoopPhase.TRAIN, data_loader=data_loader_train, epoch=epoch)
        main_loop_node(phase=LoopPhase.TRAIN, data_loader=data_loader_train, epoch=epoch)

        # Validation loop
        with torch.no_grad():
            try:
                main_loop_graph(phase=LoopPhase.VAL, data_loader=data_loader_val, epoch=epoch)
            except Exception as e:  # "patience has run out" exception :O
                print(str(e))
                break  # break out from the training loop
            precision, recall, accuracy, micro_f1 = main_loop_graph(phase=LoopPhase.TEST, data_loader=data_loader_test)
        ...
        scheduler_node.step()
        print(f"EPOCH {epoch} learning rate: {optimizer_graph.param_groups[0]['lr']:.6f}")
        print('=' * 50)
    ...
    gat.load_state_dict(torch.load(
            os.path.join(CHECKPOINTS_PATH, f'gat_{config["dataset_name"]}_{config["task_name"]}_best.pth'))['state_dict'], device)
    precision, recall, accuracy, micro_f1 = main_loop_graph(phase=LoopPhase.TEST, data_loader=data_loader_test)
...
def get_main_loop(config, gat, loss_func, optimizer, patience_period, time_start, target='graph', alpha=0.75):
    ...
    def main_loop(phase, data_loader, epoch=0):
        ...
        bar = tqdm.tqdm(data_loader, desc=f"{phase.name}-{target}")
        ...
        for mini_batch_idx, (node_features, gt_graph_labels, edge_index, node_to_graph_map, rpst_labels) in enumerate(bar):
            ...
            filter_graph_indices = torch.nonzero(gt_graph_labels.squeeze(-1) == 1., as_tuple=False).squeeze(-1)
            node_indices = torch.isin(node_to_graph_map, filter_graph_indices)
            node_indices = set(torch.nonzero(node_indices, as_tuple=False).squeeze(-1).cpu().tolist())
            rpst_indices = set(torch.nonzero(rpst_labels.squeeze(-1) != -1., as_tuple=False).squeeze(-1).cpu().tolist())
            ...
            if target == 'graph':
                ...
                # print("3", filter_rpst_labels.shape)
                loss = loss_func(filter_node_unnormalized_scores, filter_rpst_labels)
            ...
            total_graph_preds.extend((graph_unnormalized_scores.squeeze(-1) > label_theshold).int().cpu().tolist())
            total_graph_gts.extend(gt_graph_labels.squeeze(-1).int().cpu().tolist())
            # print("preds", ((filter_node_unnormalized_scores.squeeze() > label_theshold).int()==1).sum(), ((filter_node_unnormalized_scores.squeeze() > label_theshold).int()==0).sum())
            # print("lbls", (filter_rpst_labels.squeeze().int()==1).sum(), (filter_rpst_labels.squeeze().int()==0).sum())
            ...
            if is_show_this_time:
                ...
                if target == 'graph':
                    batch_micro_f1 = f1_score(
                        total_graph_gts[(mini_batch_idx+1)*config['run']['loader_batch_size']-config['run']['batch_size']:],
                        total_graph_preds[(mini_batch_idx+1)*config['run']['loader_batch_size']-config['run']['batch_size']:],
                        average='binary')
                    bar.set_postfix({"last_update": last_update, "loss": batch_mean_loss,
                                     f"{phase.name}_f1": batch_micro_f1})
                else:
                    batch_node_gts = total_node_gts[(mini_batch_idx + 1) * config['run']['loader_batch_size'] - config['run']['batch_size']:]
                    # batch_micro_f1 = f1_score(
                    #     batch_node_gts,
                    #     total_node_preds[(mini_batch_idx + 1) * config['run']['loader_batch_size'] - config['run']['batch_size']:],
                    #     average='weighted', sample_weight=compute_sample_weight(class_weight='balanced', y=batch_node_gts))
                    batch_micro_f1 = f1_score(
                        batch_node_gts,
                        total_node_preds[(mini_batch_idx + 1) * config['run']['loader_batch_size'] - config['run']['batch_size']:],
                        average='binary')
                ...
                if config['run']['enable_tensorboard']:
                    ...
                    if phase == LoopPhase.TRAIN:
                        writer.add_scalar(f'training_loss_{target}', batch_mean_loss, global_step)
                        writer.add_scalar('training_micro_f1', batch_micro_f1, global_step)
                    elif phase == LoopPhase.VAL:
                        writer.add_scalar(f'val_loss', batch_mean_loss, global_step)
                        writer.add_scalar('val_micro_f1', batch_micro_f1, global_step)

        # calculate average metrics
        # total_graph_gts = np.array(total_graph_gts)
        # total_graph_preds = np.array(total_graph_preds)
        # total_node_gts = np.array(total_node_gts)
        # total_node_preds = np.array(total_node_preds)
        # print(total_graph_gts.shape, total_graph_preds.shape, total_node_gts.shape, total_node_preds.shape)
        mean_loss = np.array(total_loss_list).mean()
        # print(total_graph_gts)
        # print(total_graph_preds)
        precision1 = precision_score(total_graph_gts, total_graph_preds, average='binary')
        recall1 = recall_score(total_graph_gts, total_graph_preds, average='binary')
        accuracy1 = accuracy_score(total_graph_gts, total_graph_preds)
        mean_micro_f11 = f1_score(total_graph_gts, total_graph_preds, average='binary')
        # sample_weight = compute_sample_weight(class_weight='balanced', y=total_node_gts)
        # precision2 = precision_score(total_node_gts, total_node_preds, average='weighted', sample_weight=sample_weight)
        # recall2 = recall_score(total_node_gts, total_node_preds, average='weighted', sample_weight=sample_weight)
        # accuracy2 = accuracy_score(total_node_gts, total_node_preds, sample_weight=sample_weight)
        # mean_micro_f12 = f1_score(total_node_gts, total_node_preds, average='weighted', sample_weight=sample_weight)
        precision2 = precision_score(total_node_gts, total_node_preds, average='binary')
        recall2 = recall_score(total_node_gts, total_node_preds, average='binary')
        accuracy2 = accuracy_score(total_node_gts, total_node_preds)
        mean_micro_f12 = f1_score(total_node_gts, total_node_preds, average='binary')
        precision = 0.7 * precision1 + 0.3 * precision2
        recall = 0.7 * recall1 + 0.3 * recall2
        accuracy = 0.7 * accuracy1 + 0.3 * accuracy2
        mean_micro_f1 = 0.7 * mean_micro_f11 + 0.3 * mean_micro_f12

        perf = {
            "loss": mean_loss,
            "Gprecision": precision1,
            "Grecall1": recall1,
            "Gaccuracy1": accuracy1,
            "Gf1": mean_micro_f11,
            "Nprecision": precision2,
            "Nrecall2": recall2,
            "Naccuracy2": accuracy2,
            "Nf1": mean_micro_f12,
            "precision": precision,
            "recall": recall,
            "accuracy": accuracy,
        ...
        if phase == LoopPhase.TRAIN:
            ...
            if config['run']['console_log_freq'] is not None and (epoch + 1) % config['run']['console_log_freq'] == 0:
                print(f'GAT training: time elapsed= {(time.time() - time_start):.2f} [s] |'
                      ...
                      f' train recall={recall:.3f} | train accuracy={accuracy:.3f} | train mean-micro-F1={mean_micro_f1:.3f} with {mean_micro_f11}:{mean_micro_f12}.')
            # Save model checkpoint
            if config['run']['checkpoint_freq'] is not None and (epoch + 1) % config['run']['checkpoint_freq'] == 0:
                ckpt_model_name = f'gat_{config["dataset_name"]}_ckpt_{config["task_name"]}_epoch_{epoch + 1}.pth'
            ...
            if config['run']['console_log_freq'] is not None and (epoch + 1) % config['run']['console_log_freq'] == 0:
                print(f'GAT validation: time elapsed= {(time.time() - time_start):.2f} [s] |'
                      ...
                      f' val recall={recall:.3f}  | val accuracy={accuracy:.3f} | val mean-micro-F1={mean_micro_f1:.3f} with {mean_micro_f11}:{mean_micro_f12}.')
            ...
            if mean_micro_f1 >= BEST_VAL_MICRO_F1 or (mean_micro_f1 == BEST_VAL_MICRO_F1 and mean_loss < BEST_VAL_LOSS):
                ...
                ckpt_model_name = f'gat_{config["dataset_name"]}_{config["task_name"]}_best.pth'
                torch.save(typist(config, perf, gat.state_dict()), os.path.join(CHECKPOINTS_PATH, ckpt_model_name))
