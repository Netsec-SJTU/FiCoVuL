...
from configure import DOXYGEN_OUTPUT_JSON_PATH, DOXYGEN_OUTPUT_RESULT_CVE_PATH, DOXYGEN_LOG_DIR_PATH, \
    DOXYGEN_OUTPUT_JSON_CVE_PATH
...
def process_single_instance(data):
    ...
    call_paths = list(filter(lambda cp: funcs[cp[0]]['label'] is not None and funcs[cp[-1]]['label'] is not None, call_paths))
    ...
    for cp in enumerate(call_paths):
        ...
        for i in reversed(range(1, len(call_path))):
            ...
            fucer = copy.deepcopy(functions[caller_id])
        ...
        if annotation['depth'] != len(annotation['call_path']) - 1:
            print("Length not equal")
            print(annotation['call_path'])
            print(call_path)
            raise Exception("Length not equal")

        if annotation['call_path'] not in used_call_path:
            # print(annotation['call_path'], annotation['depth'])
            used_call_path.append(annotation['call_path'])
            ...
            with open(DOXYGEN_OUTPUT_RESULT_CVE_PATH / f"{random_id}.c", 'w') as f:
                f.write(result)
            with open(DOXYGEN_OUTPUT_RESULT_CVE_PATH / f"{random_id}.json", 'w') as f:
                json.dump(annotation, f)
            res_files.append(random_id)
        else:
            print("Same call path")
    ...
    lock.acquire()
    store_result[json_file] = res_files
...
if __name__ == '__main__':
    files = list(DOXYGEN_OUTPUT_JSON_CVE_PATH.glob('*.json'))
    content = []
    all_change = []
    if os.path.exists(DOXYGEN_LOG_DIR_PATH / 'FF_CVE.json'):
        with open(DOXYGEN_LOG_DIR_PATH / 'FF_CVE.json', 'r') as f:
            content = json.load(f)
        finished_files = content.keys()
        files = list(filter(lambda x: x not in finished_files, files))
    if os.path.exists(DOXYGEN_LOG_DIR_PATH / 'CVE_all_change.pkl'):
        with open(DOXYGEN_LOG_DIR_PATH / 'CVE_all_change.pkl', 'rb') as f:
    ...
    with open(f'{DOXYGEN_LOG_DIR_PATH}/FF_CVE.json', 'w') as f:
        json.dump(record, f)
    with open(f'{DOXYGEN_LOG_DIR_PATH}/CVE_all_change.pkl', 'wb') as f:
