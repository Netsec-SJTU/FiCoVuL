...
def vertexToStr(vertex: Vertex, identifiers: Map[Vertex,Int]): String = {
  ...
  str.append("joern_column_")
  try {
    str.append("("+vertex.value2(NodeKeys.COLUMN_NUMBER).toString+")")
  }catch {
    case _: Throwable => str.delete(0, str.length())
    }
...
@main def main(cpgFile: String, outputFile: String) = {
  loadCpg(cpgFile)
  val obj = toDot(cpg.graph)
  val writer = new PrintWriter(new JFile(outputFile))
  writer.println(obj)
  writer.close()
}
